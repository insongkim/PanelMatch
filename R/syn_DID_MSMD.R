syn_DID_MSMD <- function (L, F, M = 1, time.id = "year", unit.id = "ccode", treatment, 
                          covariate, dependent, data) 
{
 ...
 #                                                    1), smallerlist)
  ...
  all.diffs.MSMD <- lapply(even_smaller1, MSMD_result, L = L, F, M = M)
  ...
  all.diffs.MSMD2 <- lapply(even_smaller2, MSMD_result, L = L, F, M = M)
