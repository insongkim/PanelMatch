...
cscwdid_tmp2 <- function(x, L, FORWARD) {
  L <- L; FORWARD <- FORWARD
  testid <- unique(x$V2)
  timeid <- unique(x$V1)
  ...
  if (nrow(x) > 2*(L+FORWARD+1)) {
    ...
    second.diff.1 <- sum(x[x$V2 %in% testid[-2] & x$V1 == unique(x$V1)[L+1+FORWARD], ]$V4*
                           x[x$V2 %in% testid[-2] & x$V1 == unique(x$V1)[L+1+FORWARD], ]$w.weight)
    second.diff.2 <- sum(x[x$V2 %in% testid[-2] & x$V1 == unique(x$V1)[L], ]$V4*
                           x[x$V2 %in% testid[-2] & x$V1 == unique(x$V1)[L], ]$w.weight)
    second.diff <- second.diff.1 - second.diff.2                    
    return(first.diff - second.diff)
  } else {
...
callSynth_tmp2 <- function (x, unit.id, time.id, L, FORWARD, d2) {
  testid <- unique(x$V2)
  timeid <- unique(x$V1)
  covariate.names <- colnames(x)[5:length(x)]
  if (nrow(x) > 2*(L+FORWARD+1)) {
