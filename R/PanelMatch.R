...
#' @param placebo.test logical TRUE/FALSE. indicates whether or not you want to be able to run a placebo test. This will add additional requirements on the data -- specifically, it requires that no unit included in the matching/refinement process can have missing outcome data over the lag window. As a result, configurations that have different values for this parameter but are otherwise identical may still produce different results. Additionally, you should not use the outcome variable in refinement when \code{placebo.test = TRUE}.
