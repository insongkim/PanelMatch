syn_DID_check_tmp <- function(L, FORWARD, time.id = "year", qoi = "ATE",
                          unit.id = "ccode",
  ...
  # brute forcing qoi
  all.diffs.weighted <- lapply(even_smaller1, cscwdid_tmp2, L = L, FORWARD = FORWARD)
  ...
  if (qoi == "ATT") {
    ...
    # brute forcing qoi
    all.diffs.weighted2 <- lapply(even_smaller2, cscwdid_tmp2, L = L, FORWARD = FORWARD)
