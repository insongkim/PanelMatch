MSMD_DID <- function (L, F, M = 1, time.id = "year", unit.id = "ccode", treatment, 
                          covariate, dependent, data, qoi = "ate") 
{
  ...
  all.diffs.MSMD <- sapply(even_smaller1, MSMD_result, L = L, F, M = M)
  ...
  if (qoi == "att") {
    ...
    #                                         x$V1 == unique(x$V1)[L], ]$V3 == 0, smallerlist)
    all.diffs.MSMD2 <- sapply(even_smaller2, MSMD_result, L = L, F, M = M)
