  ...
  if (bias.corrected)
  {
    ...
    df <- rbind(t(as.data.frame(object$estimates)), # point estimate
                ...
                apply(object$bootstrapped.estimates, 2, 
                      ...
                      na.rm = T), 
                2*object$estimates - colMeans(object$bootstrapped.estimates, 
                                              na.rm = T), 
  ...
  else
  {
    if (!is.null(object$bootstrap.iteration))
    {
      df <- rbind(t(as.data.frame(object$estimates)), # point estimate
                  ...
                  apply(object$bootstrapped.estimates, 2, sd, na.rm = T), # bootstrap se
                  apply(object$bootstrapped.estimates, 2, quantile, # Efron & Tibshirani 1993 p170 - 171
                        probs = c( (1 - object$confidence.level)/2, 
