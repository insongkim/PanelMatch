syn_DID_weights_tmp2 <- function(L, FORWARD, time.id = "year", qoi = "ate",
                                unit.id = "ccode",
  ...
  # calibrating weights
  weights_and_dits <- lapply(even_smaller1, callSynth_tmp2, unit.id = unit.id, time.id = time.id,
                             d2 = d2, L = L, FORWARD = FORWARD)
  ...
  if (qoi == "att") {
    ...
    # calibrating weights
    weights_and_dits <- lapply(even_smaller2, callSynth_tmp2, unit.id = unit.id, time.id = time.id,
                               d2 = d2, L = L, FORWARD = FORWARD)
    all.weights <- (lapply(weights_and_dits, function (x) x$w.weight))
    all.dits <- (lapply(weights_and_dits, function (x) x$dit))
