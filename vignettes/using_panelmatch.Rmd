...
## Examples and `matched.set` objects
...
The `PanelMatch` function returns a `PanelMatch` object, which contains a `matched.set` object. These objects are just lists with some additional attributes. Within the `PanelMatch` object, the attached `matched.set` object is always named either `att` or `atc`. When `qoi = ate`, then there are two `matched.set` objects included in the results of the `PanelMatch` call. Specifically, there will be two matched sets named `att` and `atc`, respectively.
...
We can print information about the matched sets. Matched set objects are implemented as lists, but the default printing behavior resembles that of a data frame. One can toggle a `verbose` option on the `print` method to print as a list and also display a less summarized version of the matched set data.
```{r}
names(msets) #get the treated unit ids, times of treatment
print(msets)
```

Since `matched.set` objects are just lists with attributes, you can expect the `[` and `[[` functions to work similarly to how they would with a list. So, for instance, users can extract information about matched sets using numerical indices or by taking advantage of the naming scheme. Each matched set will have an attribute called "weights," which are the weights calculated for each control unit in the refinement process. Here, because no refinment was used, each control unit has equal weight. 
```{r}
msets[1] #prints like a matched.set object
msets[[1]] #prints the matched control unit ids.
attr(msets[[1]], "weights")
```

We can also get an idea of the number and size of our matched sets by using the `summary` and `plot` methods for `matched.set` objects. By default, the `plot` method excludes empty matched sets (treated units that could not be matched to any control units), showing the number of empty sets in a separate vertical bar at x = 0. This can be turned off by setting `include.empty.sets` to `TRUE`.
...
plot(PM.results.none$att, include.empty.sets = TRUE) 
#The red tiny bar that would otherwise indicate empty sets is now part of the grey bar
```

### Other methods of refinement


Below, we will use the `mahalanobis` option for `refinement.method` and will use only contemporaneous values of the `tradewb` variable to define similarity.
...
# Investigating and Evaluating Matched Set Data

### DisplayTreatment with matched set data
...
### Evaluating refinement with `get_covariate_balance`

Now, let's see the impact of performing refinement by comparing the balance of covariates before and after. See the documentation for the `get_covariate_balance` function for more about how the package calculates covariate balance. We hope to see low values (.2 is a helpful threshold value as a rule of thumb) in the results of our covariate balance calculations. We also hope to see an improvement in covariate balance after applying some method of refinement to our results. 
...
```{r}
get_covariate_balance(matched.sets = PM.results1$att,
                      data = dem,
                      covariates = c("tradewb", "y"), 
